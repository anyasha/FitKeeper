...
class ExerciseViewController: UIViewController {
            ...
            ref.observeSingleEvent(of: .value, with: { snapshot in
                if ( snapshot.value is NSNull ) {
                    ...
                    if let dict = snapshot.value as? NSDictionary, let info = dict["kcal"] as? String {
                        self.kcal.text = info + " kcal"
                        self.kcals = info
                    }
                    if let dict = snapshot.value as? NSDictionary, let info = dict["mins"] as? String {
                        self.min.text = info + " min"
                        self.mins = info
                    }
                    if let dict = snapshot.value as? NSDictionary, let info = dict["type"] as? String {
                        self.type = info
                    }
    ...
    @IBAction func add(_ sender: UIButton) {
        if isUsed == false {
            let dest = self.type == "exercise" ? "activities" : "meals"
            if  self.type == "exercise" {
                var burned = 0
                if let uid = Auth.auth().currentUser?.uid{
                    let ref = Database.database().reference(fromURL: "https://fitkeeper-e9117.firebaseio.com/").child("users").child(uid).child("dairy").child(getToday()).child("balance")
                    ref.observeSingleEvent(of: .value, with: { snapshot in
                        if !snapshot.exists() {  return }
                        if let dict = snapshot.value as? NSDictionary, let info = dict["burned"] as? String {
                            burned = Int(info)!
                            burned += Int(self.kcals)!
                            ref.updateChildValues(["burned" : String(burned)])
                        }
                    })
                }
            } else {
                ...
                if let uid = Auth.auth().currentUser?.uid{
                    ...
                    ref.observeSingleEvent(of: .value, with: { snapshot in
                        ...
                        if let dict = snapshot.value as? NSDictionary, let info = dict["eaten"] as? String {
                            burned = Int(info)!
                            burned += Int(self.kcals)!
                            ref.updateChildValues(["eaten" : String(burned)])
                        }
                    })
                }
            }
