//
//  MarathoneMembersTableViewController.swift
//  FitKeeper
...
class MarathoneMembersTableViewController: UITableViewController {
   ...
   var vocabulary = ["On-time completed tasks", "Completed tasks", "Losted weight", "Losted chest girth", "Losted waist girth", "Losted hip girth"]
    ...
    @IBOutlet weak var typeMems: UITextField!
    @IBOutlet weak var amontMem: UITextField!
...
//        self.navigationController?.navigationBar.isTranslucent = false
      ...
      //   self.tableView.isEditing = true
        ...
        if let uid = Auth.auth().currentUser?.uid{
            var ref = Database.database().reference(fromURL: "https://fitkeeper-e9117.firebaseio.com/").child("members").child(self.marathone)
            ref.observeSingleEvent(of: .value, with: { snapshot in
                if !snapshot.exists() { return }
                if let dict = snapshot.value as? NSDictionary{
                    self.members = dict.allKeys as! [String]
                    self.allMembers.removeAll()
                    self.amontMem.text = String(self.members.count)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let a = indexPath.row
        performSegue(withIdentifier: "MemberDetails", sender:  self.allMembers[indexPath.row]["uid"] as! String)
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MemberDetails" {
            let guest = segue.destination as! MemberTableViewController
            let a = sender as! String
            guest.member = sender as! String
            guest.marathone = self.marathone
        }
        if segue.identifier == "rankSegue" {
//            let nav = segue.destination as! UINavigationController
//            let guest = nav.topViewController as! MemberTableViewController
            let guest = segue.destination as!  MarathoneRankTableViewController
            guest.marathone = self.marathone
        }
    }
    
    @IBAction func saveWinners(_ sender: UIBarButtonItem) {
        var i = 1
        for m in self.allMembers {
            if let uid = Auth.auth().currentUser?.uid{
                Database.database().reference(fromURL: "https://fitkeeper-e9117.firebaseio.com/").child("winners").child(self.marathone).updateChildValues([m["uid"] as! String : String(i)])
            i += 1
        }
        
    }
         self.signupErrorAlert(title: "Thank you!", message: "All the winners are approved.")
    ...
    override func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        let itemToMove =  self.allMembers[sourceIndexPath.row]
        self.allMembers.remove(at: sourceIndexPath.row)
        self.allMembers.insert(itemToMove, at: destinationIndexPath.row)
    ...
    @IBAction func unwindToMembers(storyboard:UIStoryboardSegue){
    }
}
